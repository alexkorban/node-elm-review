...
async function checkForUncommittedSuppressions(options) {
    try {
        return childProcess.execSync(`git diff-index HEAD ${options.suppressedErrorsFolder()}`);
    }
    catch (error) {
        if (error.status === 1) {
            console.log(`You have uncommitted changed in ${chalk.keyword('orange')(path.relative(process.cwd(), options.suppressedErrorsFolder()))}.`)
            process.exit(1);
        }
