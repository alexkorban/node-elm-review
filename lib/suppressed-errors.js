...
async function checkForUncommittedSuppressions(options) {
    const pathToSuppressedFolder = path.relative(process.cwd(), options.suppressedErrorsFolder());
    try {
        return childProcess.execSync(`git diff-index --quiet HEAD -- ${pathToSuppressedFolder}`);
    }
    catch (error) {
        if (error.status === 1) {
            console.log(`You have uncommitted changed in ${chalk.keyword('orange')(pathToSuppressedFolder)}.`)
            process.exit(1);
        }
